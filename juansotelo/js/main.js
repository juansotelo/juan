...
function onDeviceReady(){
	...
	db = window.openDatabase("productos", "1.0", "DB del curso Phonegap", 200000);
	if(existe_db == null){
		creaDB();
	}else{
		cargaDatos();
...
function creaNuevaDB(tx){
	...
	tx.executeSql("INSERT INTO productos (id,nombre,descripción,precio,categoria,foto,existencia) VALUES (1,'MacPro','Genial','30000','apple','mac.jpg','100')");
}
...
function cargaRegistros(tx){
	...
	tx.executeSql('SELECT * FROM productos', [], cargaDatosSuccess, errorDB);
}
...
function cargaDatosSuccess(tx, results){
	...
	if(results.rows.length == 0){
		...
		navigator.notification.alert("No hay productos en la base de datos");
	}
	...
	for(var i=0; i<results.rows.length; i++){
		...
		selector.append('<li id="li_'+persona.id+'"><a href="#detalle" data-uid='+persona.id+' class="linkDetalles"><div class="interior_lista"><img src="assets/'+ foto +'" class="img_peq"/><span>' + persona.nombre + ' ' + persona.descripción+ '</span></div></a><a href="#form"  data-theme="a" data-uid='+persona.id+'  class="linkForm">Predet.</a></li>').listview('refresh');
	}
...
function queryDBFindByID(tx) {
    tx.executeSql('SELECT * FROM productos WHERE id='+$.id, [], queryDetalleSuccess, errorDB);
}
...
function queryDetalleSuccess(tx, results) {
	...
	$("#categoria").html($.registro.categoria);
		...
		$("#nombre").html($.registro.nombre + " " + $.registro.descripción);
		$("#num_tel").html($.registro.precio);
		
		$("#label_mail").html("existencia: " + $.registro.existencia);
}
...
function queryDBFindByIDForm(tx) {
    tx.executeSql('SELECT * FROM productos WHERE id='+$.id, [], queryFormSuccess, errorDB);
}
...
function queryFormSuccess(tx, results) {
	...
	$.registro = results.rows.item(0);
		...
		$("#ti_descripción").val($.registro.descripción);
		$("#ti_precio").val($.registro.precio);
		$("#ti_existencia").val($.registro.existencia);
...
function initForm(){
	...
	$("#cat_apple").trigger("click").trigger("click")
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE clientes SET nombre="'+$("#ti_nombre").val()+'", descripción="'+$("#ti_descripción").val()+'",Precio="'+$("#ti_precio").val()+'",existencia="'+$("#ti_existencia").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
function updateFormSuccess(tx) {
	...
	selector.find("a:first").find("span").html($("#ti_nombre").val() + " " + $("#ti_descripción").val());
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO clientes (nombre,descripción,precio,categoria,foto,existencia) VALUES ('"+$("#ti_nombre").val()+"','"+$("#ti_descripción").val()+"','"+$("#ti_precio").val()+"','"+cat+"','"+$.imageURL+"','"+$("#ti_existencia").val()+"')", [], newFormSuccess, errorDB);
}
function newFormSuccess(tx, results) {
	...
	var obj = $('<li id="li_'+results.insertId+'"><a href="#detalle" data-uid='+results.insertId+' class="linkDetalles"><div class="interior_lista"><img src="'+ $.imageUR +'" class="img_peq"/><span>' + $("#ti_nombre").val() + " " + $("#ti_descripción").val()+ '</span></div></a><a href="#form"  data-theme="a" data-uid='+results.insertId+'  class="linkForm">Predet.</a></li>');
